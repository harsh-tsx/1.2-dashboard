...
import { PlantRoleData } from '@/api-client/plant/models'
import { PlantRoleService } from '@/api-client/PlantApi'



export type Role = PlantRoleData['responses']['List']['data'][0]
    ...
    (set, get) => ({
      get: {
        list: async () => {
          ...
          toast.promise(PlantRoleService.list({
            query: {
              page: page as any,
              size: size as any,
            }
      ...
      add: async (bodyData: PlantRoleData['payloads']['Create']['requestBody']) => {
        let id = get().role.id
        let isUser = get().role.isUser

        toast.promise(
          id
            ? PlantRoleService.update({
              query: {
                id: id,
              },
              requestBody: bodyData as any,
            }) : PlantRoleService.create({
      ...
      delete: async () => {
        ...
        toast.promise(PlantRoleService.delete({ query: { id: id } }), {
          loading: 'deleting',
          success: res => {
            useRoleStore.getState().get.paginate({})
            useRoleStore.getState().select(null)
