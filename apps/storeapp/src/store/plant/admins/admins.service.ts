...
import { StoreAdminData } from '@/api-client/store/models'
import { StoreAdminService } from '@/api-client/PlantApi'



export type Admin = StoreAdminData['responses']['AdminList']['data'][0]
    ...
    (set, get) => ({
      get: {
        list: async () => {
          ...
          toast.promise(StoreAdminService.adminList({
            query: {
              page: page as any,
              size: size as any,
            }
      ...
      add: async (bodyData: StoreAdminData['payloads']['AdminCreate']['requestBody']) => {
        let id = get().admin.id
        let isUser = get().admin.isUser

        toast.promise(
          id
            ? StoreAdminService.adminUpdate({
              query: {
                id: id,
              },
              requestBody: bodyData as any,
            }) : StoreAdminService.adminCreate({
      ...
      delete: async () => {
        ...
        toast.promise(StoreAdminService.adminDelete({ query: { id: id } }), {
          loading: 'deleting',
          success: res => {
            useAdminStore.getState().get.paginate({})
            useAdminStore.getState().select(null)
